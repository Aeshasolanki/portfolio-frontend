...
export default function ProjectSection({ limit }: ProjectSectionProps) {
            ...
            >
              ...
              <div className="text-white space-y-6 max-w-2xl z-10">
                ...
                {project.description_secondary && (
                  <p className="text-gray-500 text-sm">{project.description_secondary}</p>
                )}
  ...
  />
      ...
      {/* Stats Section */}
 <div className="relative z-20 mt-16 mb-40 px-6 flex justify-center">
  {stats.map((stat, idx) => {
    ...
    return (
      ...
      >
        ...
        <p className="text-xs md:text-sm lg:text-lg text-zinc-400 uppercase mt-2">
          {stat.label}
        </p>
        <Icon className="mt-4 w-8 h-8 md:w-12 md:h-12 text-white/70" />
      </motion.div>
    );
  })}
</div>
